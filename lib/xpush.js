  ...
  var XPush = (function() {
    ...
    XPush.prototype.uploadFile = function(channel, fileUri, inputObj, fnPrg, fnCallback){
      ...
      self.getChannelAsync(channel, function(err, ch){
        ...
        // In device
        if( typeof window !== 'undefined' && window.FileTransfer && window.FileUploadOptions){
          ...
          var serverInfo = ch._server.serverUrl.replace("http://","").replace("https://", "" ).split( ":" );

          var hostname = serverInfo[0];
          ...
          function getPostData(value) {
            function encodeFile(boundary,type,name,filename) {
              var result = "--" + boundary + "\r\n";
              result += "Content-Disposition: form-data; name=\"" + name + "\"; filename=\"" + filename + "\"\r\n";
            ...
            if (value) {
              postData.push(new Buffer(encodeFile(boundary, value.type, value.keyname, value.valuename), 'ascii'));
          ...
          fs.readFile(fileUri, function read(err, filecontents) {
            ...
            var postData = getPostData(fileValue);
            var headerData = postData.header;
            for (var key in headerData) options.headers[key] = headerData[key];
            ...
            for (var i = 0; i < postData.params.length; i++) {
              request.write(postData.params[i]);
            }
